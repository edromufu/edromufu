...
import sys, rclpy, os, json

from movement_utils.srv import *  #Importação por ros2 definida no package.xml
from movement_utils.msg import *  #Importação por ros2 definida no package.xml
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.node = rclpy.create_node('page_interface')

        self.motorsTorque = self.node.create_client(EnableTorque, 'u2d2_comm/enableTorque')
        self.motorsFeedback = self.node.create_client(BodyFeedback, 'u2d2_comm/feedbackBody')
        self.pageRunner = self.node.create_client(Page, 'movement_central/request_page')

        while not self.motorsTorque.wait_for_service(timeout_sec=1.0):
    ...
    def toggleAllTorque(self):       
        ...
        #
        resp=self.motorsTorque.call_async(self.allTorqueStatus, list(range(20)))
    ...
    def toggleOneTorque(self, motor_id):        
        ...
        resp=self.motorsTorque.call_async(self.motorsTorqueStatus[motor_id], list([motor_id]))
    ...
    def generateNewCard(self, requestFeedback=True):
        #if requestFeedback:
        #    current_position = list(self.motorsFeedback(True).pos_vector) '''aaaa'''
        #else:
        #    current_position = [-1]*18

        if requestFeedback:
            future = self.motorsFeedback.call_async(request)
            rclpy.spin_until_future_complete(self.node, future)
            if future.result() is not None:
                current_position = list(future.result().pos_vector)
            else:
                # Lida com o caso em que a chamada do serviço falhou
                current_position = [-1] * 18
        else:
            current_position = [-1] * 18
    ...
    def playPage(self, selected=False):
        ...
        resp=self.pageRunner.call_async('temp2run')
        rclpy.spin_until_future_complete(self.node, resp)
