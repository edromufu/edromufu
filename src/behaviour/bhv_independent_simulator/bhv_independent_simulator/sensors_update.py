...
#from modularized_bhv_msgs.msg import simMovMsg (2) #Mensagem associada ao tópico utilizado para enviar info dos motores
from modularized_bhv_msgs.msg import SimMovMsg #(2) #Mensagem associada ao tópico utilizado para enviar info dos motores
from geometry_msgs.msg import Vector3 #Mensagem associada ao tópico utilizado para enviar info do acelerometro
#from sensor_msgs.msg import Image as visionSimImage (3) #Mensagem associada ao tópico utilizado para enviar as imagens da câmera
from sensor_msgs.msg import Image as VisionSimImage #(3) #Mensagem associada ao tópico utilizado para enviar as imagens da câmera
...
class RobotSensors():
    ...
    def init_head(self):
        ...
        #self.head_pos_publisher = rospy.Publisher('/webots/motors', simMovMsg, queue_size=100) (4)
        self.head_pos_publisher = self.node.create_publisher(SimMovMsg, '/webots/motors', 100) #(4)
        #self.head_pos_msg = simMovMsg() (5)
        self.head_pos_msg = SimMovMsg() #(5)
    ...
    def init_accel(self):
        ...
        #self.accel_publisher = rospy.Publisher('/webots_natasha/behaviour_controller', Vector3, queue_size=100) (7)
        self.accel_publisher = self.node.create_publisher(Vector3, '/webots_natasha/behaviour_controller', 100) #(7)
    ...
    def init_cam(self):
        ...
        #self.pubImage = rospy.Publisher('/webots_natasha/vision_controller', visionSimImage, queue_size= 33) (8)
        self.pubImage = self.node.create_publisher(VisionSimImage, '/webots_natasha/vision_controller', 33) #(8)

        #self.image_msg = visionSimImage() (9)
        self.image_msg = VisionSimImage() #(9)
