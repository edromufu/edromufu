...
from modularized_bhv_msgs.msg import CurrentStateMsg #(2)
#from modularized_bhv_msgs.srv import moveRequest, moveRequestResponse (3) #Srv associado ao service utilizado para requisitar qualquer movimento dos motores
from modularized_bhv_msgs.srv import MoveRequest, MoveRequestResponse #(3) #Srv associado ao service utilizado para requisitar qualquer movimento dos motores
...
class Robot3DMover():
    ...
    def __init__(self, supervisor):
        ...
        #rospy.Subscriber('/transitions_and_states/state_machine', currentStateMsg, self.flagUpdate) (4)
        self.node.create_subscription(CurrentStateMsg, '/transitions_and_states/state_machine', self.flagUpdate, rclpy.qos.QoSProfile()) #(4)
    ...
    def init_3D(self):
        ...
        #rospy.Service('/bhv2mov_communicator/3D_move_requisitions', moveRequest, self.moveSimRobot) (5)
        self.node.create_service(MoveRequest, '/bhv2mov_communicator/3D_move_requisitions', self.moveSimRobot) #(5)
        #self.response = MoveRequestResponse() (6) Não será mas necessário, se tornou argumento da função moveSimRobot
        
    
    #Função responsável pela movimentação 3D de acordo com as requisições
    def moveSimRobot(self, request, response): #(6)
        """
        -> Funcao:
