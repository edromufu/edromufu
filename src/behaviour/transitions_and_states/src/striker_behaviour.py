...
class striker_brain:
    def __init__(self):
        ...
        self.node = rclpy.create_node('striker_brain')

        self.parameters = BehaviourParameters()

        self.motorsFeedback = self.node.create_client(HeadFeedback, 'u2d2_comm/feedbackHead')
        #self.pageCall = self.node.create_client(Page, 'movement_central/request_page') as pages vao ser chamadas diferentes
...
"""
    def updateBallParameters(self, msg):
        ...
        if not ballInfos.found:
            ...
            self.x = ballInfos.x
            self.y = ballInfos.y
            self.x_position = ballInfos.x_position
            self.y_position = ballInfos.y_position
            self.z_position = ballInfos.z_position

            #self.ballClose = True  if self.y > self.parameters.yCenterBottomLimit else False
            self.ballClose = True  if self.z_position > self.parameters.zCenterLimit else False # Com a camera nova (detectar proximidade da bola)
            ...
            if self.found:
                ...
                if self.found and self.ballClose:
                    ...
                    if self.HorRotation < self.parameters.lookingLeftRad / 2:
                        ...
                        self.fall()
                    '''
                    self.pageCall('tahara_kick') # Mudar pageCall depois
    ...
    def fall(self):
        while rclpy.ok():
            self.pageCall('fallen_tahara') # fazer page de levantar a tahara (ele não vai levantar não ein)
                

if __name__ == '__main__':
    striker = striker_brain()
    striker.run()
    rclpy.spin(striker.node)
