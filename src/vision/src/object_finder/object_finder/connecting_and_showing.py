...
########3from behaviour_parameters import BehaviourParameters
...
class Visao(Node):
    ...
    def __init__(self,nome_no):
        ...
        #######Declara a existência dos parametros, talvez desnecessária na exeecução do launch
        self.camera = self.declare_parameter('vision/camera',2).get_parameter_value().integer_value
        self.output_img = self.declare_parameter('vision/img_output',True).get_parameter_value().bool_value
        self.ajuste = self.declare_parameter('vision/ajuste',True).get_parameter_value().bool_value
        self.bright = self.declare_parameter('vision/brilho',4).get_parameter_value().integer_value
        
        
        self.camera = self.get_parameter('vision/camera').get_parameter_value().integer_value
        self.output_img = self.get_parameter('vision/img_output').get_parameter_value().bool_value
        self.ajuste = self.get_parameter('vision/ajuste').get_parameter_value().bool_value
        self.bright = self.get_parameter('vision/brilho').get_parameter_value().integer_value
        print(f"\nCamera:{self.camera}\nOutput:{self.output_img}\nAjuste:{self.ajuste}\nBrilho:{self.bright}\n")

        '''Dá onde surge o BehaviourParameters'''
        ...
        '''ROS2 depende dos arquivos msg e topic do behaviour
        
        self.publisher = self.create_publisher(self.parameters.vision2BhvTopic, Webotsmsg, queue_size=100)
        '''
        #self.publisher = self.create_publisher('vision2BhvTopic', Webotsmsg, queue_size=100)
    ...
    def get_webcam(self):
        ...
        while rclpy.ok():
            ...
            self.classes, self.scores, self.boxes, self.fps,self.inference_frame = ri.detect_model(self.model,self.current_frame,self.output_img)
            
            
            #Para testar a eficiencia da inferencia comparando a  execução sem ela
            ...
            print(f'FPS da inferência: {self.fps}')
